...
def load_data(config, logger):
	...
	if config.mode == 'train':
		...
		train_set = TextDataset(data_path=data_path, dataset=config.dataset,
								datatype='train', max_length=config.max_length, is_debug=config.debug, to_sort=True)
...
def main():
	...
	if is_train:
		...
		logger.info(
			'Vocab1 Created with number of words : {}'.format(voc1.nwords))
		logger.info(
			'Vocab2 Created with number of words : {}'.format(voc2.nwords))
	...
	if is_train:
		...
		if checkpoint == None:
			...
			epoch_offset, min_train_loss, min_val_loss, max_train_acc, max_val_acc, max_val_bleu, max_test_acc, max_gen_acc, best_epoch, voc1, voc2 = load_checkpoint(config, model, config.mode, checkpoint, logger, device)
		...
		train_model(model, train_dataloader, val_dataloader, temp_test_dataloader, gen_dataloader, voc1, voc2,
					device, config, logger, epoch_offset, min_val_loss, max_val_bleu, max_val_acc, max_test_acc, max_gen_acc, min_train_loss, max_train_acc, best_epoch, writer)
		...
		if config.mode == 'test':
			test_acc_epoch = run_validation(config, model, test_dataloader, "TEST", voc1, voc2, device, logger, 0)
