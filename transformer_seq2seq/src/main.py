...
def load_data(config, logger):
	...
	if config.mode == 'train':
		...
		train_set = TextDataset(data_path=data_path, dataset=config.dataset, datatype='train', max_length=config.max_length, 
								is_debug=config.debug, to_sort=True, get_tag=get_tag_value, other_inp_name=config.other_inp_name, get_blank=config.blank_op)
		val_set = TextDataset(data_path=data_path, dataset=config.dataset, datatype='dev', max_length=config.max_length, 
								is_debug=config.debug, to_sort=True, get_tag=get_tag_value, other_inp_name=config.other_inp_name, get_blank=config.blank_op)
		temp_test_set = TextDataset(data_path=data_path, dataset=config.dataset, datatype='test', max_length=config.max_length, 
								is_debug=config.debug, to_sort=True, get_tag=get_tag_value, other_inp_name=config.other_inp_name, get_blank=config.blank_op)
		...
		val_dataloader = DataLoader(
			val_set, batch_size=config.batch_size, shuffle=False, num_workers=5)
		temp_test_dataloader = DataLoader(
			temp_test_set, batch_size=config.batch_size, shuffle=False, num_workers=5)
		...
		temp_test_size = len(temp_test_dataloader)* config.batch_size
		gen_size = len(gen_dataloader)* config.batch_size
		
		msg = 'All Data Loaded:\nTrain Size: {}\nVal Size: {}\nTest Size: {}\nGen Size: {}'.format(train_size, val_size, temp_test_size, gen_size)
		logger.info(msg)

		return train_dataloader, val_dataloader, temp_test_dataloader, gen_dataloader
...
def main():
	...
	if config.tag_emb or config.tag_hid:
		vocab3_path = os.path.join(config.model_path, 'vocab3.p')
	...
	if is_train:
		train_dataloader, val_dataloader, temp_test_dataloader, gen_dataloader = load_data(config, logger)
		...
		if config.tag_emb or config.tag_hid:
			voc3 = Voc()
			voc3.create_vocab_dict(config, train_dataloader, 'tag')
		else:
			voc3 = None
		...
		if config.tag_emb or config.tag_hid:
			with open(vocab3_path, 'wb') as f:
				pickle.dump(voc3, f, protocol=pickle.HIGHEST_PROTOCOL)
	...
	if is_train:
		model = build_model(config=config, voc1=voc1, voc2=voc2, voc3=voc3, device=device, logger=logger)
		...
		if checkpoint == None:
			...
			epoch_offset, min_train_loss, min_val_loss, max_train_acc, max_val_acc, max_val_bleu, max_test_acc, max_gen_acc, best_epoch, voc1, voc2, voc3 = \
		...
		train_model(model, train_dataloader, val_dataloader, temp_test_dataloader, gen_dataloader, voc1, voc2, voc3, device, config, logger, 
					epoch_offset, min_val_loss, max_val_bleu, max_val_acc, max_test_acc, max_gen_acc, min_train_loss, max_train_acc, best_epoch, writer)
		...
		epoch_offset, min_train_loss, min_val_loss, max_train_acc, max_val_acc, max_val_bleu, best_epoch, voc1, voc2, voc3 = \
																load_checkpoint(model, config.mode, checkpoint, logger, device)
		...
		test_acc_epoch = run_validation(config, model, test_dataloader, "TEST", voc1, voc2, voc3, device, logger, 0)
