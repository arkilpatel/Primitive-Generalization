...
from src.components.transformer import CustomTransformer
...
class TransformerModel(nn.Module):
	def __init__(self, config, voc1, voc2, voc3, device, logger, EOS_tag = '</s>', SOS_tag = '<s>'):
		...
		if self.config.embedding == 'bert':
			...
			if config.tag_emb or config.tag_hid:
				self.config.d_model = self.config.d_model1 + self.config.d_model2
				self.embedding1  = nn.Embedding(self.voc1.nwords, self.config.d_model1)
				nn.init.uniform_(self.embedding1.weight, -1 * self.config.init_range, self.config.init_range)
				self.embedding3  = nn.Embedding(self.voc3.nwords, self.config.d_model2)
				nn.init.uniform_(self.embedding3.weight, -1 * self.config.init_range, self.config.init_range)
			else:
				self.embedding1  = nn.Embedding(self.voc1.nwords, self.config.d_model)
				nn.init.uniform_(self.embedding1.weight, -1 * self.config.init_range, self.config.init_range)

		if config.tag_hid:
			self.pos_embedding1 = PositionalEncoding(self.config.d_model1, self.config.dropout)
			self.pos_embedding3 = PositionalEncoding(self.config.d_model2, self.config.dropout)
		else:
		...
		if config.tag_hid:
			self.transformer = CustomTransformer(d_model1=self.config.d_model1, d_model2=self.config.d_model2, nhead=self.config.heads, 
	...
	def _initialize_optimizer(self):
		if self.config.tag_emb:
			self.params = list(self.embedding1.parameters()) + list(self.embedding3.parameters()) + list(self.transformer.parameters()) + list(self.fc_out.parameters()) + \
							list(self.embedding2.parameters()) + list(self.pos_embedding1.parameters()) + list(self.pos_embedding2.parameters())
		elif self.config.tag_hid:
			self.params = list(self.embedding1.parameters()) + list(self.embedding3.parameters()) + list(self.transformer.parameters()) + list(self.fc_out.parameters()) + \
							list(self.embedding2.parameters()) + list(self.pos_embedding1.parameters()) + list(self.pos_embedding2.parameters()) + \
							list(self.pos_embedding3.parameters())
		else:
			self.params = list(self.embedding1.parameters()) + list(self.transformer.parameters()) + list(self.fc_out.parameters()) + \
							list(self.embedding2.parameters()) + list(self.pos_embedding1.parameters()) + list(self.pos_embedding2.parameters())

		if self.config.tag_hid:
			self.non_emb_params = list(self.transformer.parameters()) + list(self.fc_out.parameters()) + list(self.embedding2.parameters()) + \
								list(self.pos_embedding1.parameters()) + list(self.pos_embedding2.parameters()) + list(self.pos_embedding3.parameters())
		else:
			self.non_emb_params = list(self.transformer.parameters()) + list(self.fc_out.parameters()) + list(self.embedding2.parameters()) + \
	...
	def forward(self, ques, src, trg, tag):
		'''
			Args:
		...
		if self.config.embedding == 'bert' or self.config.embedding == 'roberta':
			...
			if self.config.tag_emb:
				src1 = self.embedding1(src)
				src2 = self.embedding3(tag)
				src = torch.cat((src1, src2), 2)
				src = self.pos_embedding1(src)
			elif self.config.tag_hid:
				tag_pad_mask = self.make_len_mask(tag)
				src = self.embedding1(src)
				src = self.pos_embedding1(src)
				tag = self.embedding3(tag)
				tag = self.pos_embedding3(tag)
			else:
				src = self.embedding1(src)
				src = self.pos_embedding1(src)
		...
		if self.config.tag_hid:
			output = self.transformer(src, tag, trg, src_mask=self.src_mask, tag_mask=self.tag_mask, tgt_mask=self.trg_mask, memory_mask=self.memory_mask,
										src_key_padding_mask=src_pad_mask, tag_key_padding_mask=tag_pad_mask, tgt_key_padding_mask=trg_pad_mask, 
	...
	def trainer(self, src, input_seq1, input_seq2, input_seq3, config, device=None ,logger=None):
		'''
			Args:
		...
		output = self.forward(src, input_seq1, input_seq2[:-1,:], input_seq3)
	...
	def greedy_decode(self, src=None, input_seq1=None, input_seq2=None, input_seq3=None, input_len2 = None, validation=False):
		'''
			Args:
				src (list): raw source input | size : [BS]
				input_seq1 (tensor): source indices | size : [S x BS]
		...
		with torch.no_grad():
			...
			if self.config.embedding == 'bert' or self.config.embedding == 'roberta':
				...
				if self.config.tag_emb:
					src1 = self.embedding1(input_seq1)
					src2 = self.embedding3(input_seq3)
					src = torch.cat((src1, src2), 2)
					memory = self.transformer.encoder(self.pos_embedding1(src))
				elif self.config.tag_hid:
					memory1 = self.transformer.encoder1(self.pos_embedding1(self.embedding1(input_seq1)))
					memory2 = self.transformer.encoder2(self.pos_embedding3(self.embedding3(input_seq3)))
...
def build_model(config, voc1, voc2, voc3, device, logger):
	'''
		Args:
	...
	model = TransformerModel(config, voc1, voc2, voc3, device, logger)
	model = model.to(device)

	return model

def train_model(model, train_dataloader, val_dataloader, temp_test_dataloader, gen_dataloader, voc1, voc2, voc3, device, config, logger, epoch_offset= 0, min_val_loss=float('inf'), 
				max_val_bleu=0.0, max_val_acc = 0.0, max_test_acc = 0.0, max_gen_acc = 0.0, min_train_loss=float('inf'), max_train_acc = 0.0, best_epoch = 0, writer= None):
	'''
		Args:
			model (object of class TransformerModel): model
	...
	for epoch in range(1, config.epochs + 1):
		...
		for data in train_dataloader:
			...
			sent3s = None
			if config.tag_emb or config.tag_hid:
				sent3s = sents_to_idx(voc3, data['tag'], config.max_length, flag=0)
			sent2s = sents_to_idx(voc2, data['trg'], config.max_length, flag=1)
			sent1_var, sent2_var, sent3_var, input_len1, input_len2  = process_batch(sent1s, sent2s, sent3s, voc1, voc2, voc3, device)

			model.train()

			loss = model.trainer(src, sent1_var, sent2_var, sent3_var, config, device, logger)
			train_loss_epoch += loss

			if config.show_train_acc:
				model.eval()

				_, decoder_output = model.greedy_decode(src, sent1_var, sent2_var, sent3_var, input_len2, validation=True)
		...
		if config.dev_set and (config.dev_always or epoch >= config.epochs - (config.eval_last_n - 1)):
			val_bleu_epoch, val_loss_epoch, val_acc_epoch = run_validation(config=config, model=model, val_dataloader=val_dataloader, disp_tok='DEV',
																	voc1=voc1, voc2=voc2, voc3=voc3, device=device, logger=logger, epoch_num = epoch)
		else:
			val_bleu_epoch = [0.0]
			val_loss_epoch = float('inf')
			val_acc_epoch = 0.0
		
		if config.test_set and (not config.dev_only and (config.test_always or epoch >= config.epochs - (config.eval_last_n - 1))):
			_, _, test_acc_epoch = run_validation(config=config, model=model, val_dataloader=temp_test_dataloader, disp_tok='TEST', voc1=voc1, voc2=voc2, voc3=voc3, device=device, logger=logger, epoch_num = epoch)
		else:
			test_acc_epoch = 0.0

		if not config.dev_only and (config.gen_always or epoch >= config.epochs - (config.eval_last_n - 1)):
			_, gen_acc_epoch = run_validation(config=config, model=model, val_dataloader=gen_dataloader, disp_tok='GEN', voc1=voc1, voc2=voc2, voc3=voc3, device=device, logger=logger, epoch_num = epoch)
		else:
			gen_acc_epoch = 0.0
	...
	if config.results:
		store_results(config, max_val_bleu, max_val_acc, max_test_acc, max_gen_acc, min_val_loss, max_train_acc, min_train_loss, best_epoch)
		logger.info('Scores saved at {}'.format(config.result_path))

	return max_val_acc

def run_validation(config, model, val_dataloader, disp_tok, voc1, voc2, voc3, device, logger, epoch_num, validation = True):
	'''
		Args:
			config (dict): command line arguments
			model (object of class TransformerModel): model
	...
	for data in val_dataloader:
		...
		if disp_tok == "GEN":
			decoder_output = model.greedy_decode(src, sent1_var, sent2_var, sent3_var, input_len2, validation=False)
		else:
			val_loss, decoder_output = model.greedy_decode(src, sent1_var, sent2_var, sent3_var, input_len2, validation=True)
		...
		if config.tag_emb or config.tag_hid:
			sent3s = idx_to_sents(voc3, sent3_var, no_eos= True)
		...
		with open(config.outputs_path + '/outputs.txt', 'a') as f_out:
			...
			for i in range(len(sent1s[:display_n])):
				try:
					...
					if config.tag_emb or config.tag_hid:
						f_out.write('Tag: ' + stack_to_string(sent3s[i]) + '\n')
	...
	if disp_tok == "GEN":
		return val_bleu_epoch, val_acc_epoch
