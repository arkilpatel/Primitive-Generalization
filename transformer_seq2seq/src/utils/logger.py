...
def store_results(config, max_val_bleu, max_val_acc, max_test_acc, max_gen_acc, min_val_loss, max_train_acc, min_train_loss, best_epoch):
	try:
	...
	with FileLock(config.result_path + '.lock'):
		...
		try:
			...
			, 'opt' : config.opt
			, 'blank_op': config.blank_op
			, 'tag_hid': config.tag_hid
			, 'tag_emb': config.tag_emb
			}
