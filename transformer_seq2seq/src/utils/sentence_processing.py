...
def process_batch(sent1s, sent2s, sent3s, voc1, voc2, voc3, device):
	...
	sent3s_padded = None
	if sent3s is not None:
		sent3s_padded = [pad_seq(s, max_length_1, voc3) for s in sent3s]
	...
	sent3_var = None
	if sent3s_padded is not None:
		sent3_var = Variable(torch.LongTensor(sent3s_padded)).transpose(0, 1)
		sent3_var = sent3_var.to(device)


	sent1_var = sent1_var.to(device)
	sent2_var = sent2_var.to(device)

	return sent1_var, sent2_var, sent3_var, input_len1, input_len2
